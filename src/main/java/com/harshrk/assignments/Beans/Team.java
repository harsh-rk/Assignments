...
public class Team {
    ...
    public static Team getTeam(String name) {
        ...
        for(int i=0; i< MatchConstants.NUMBER_OF_BATSMEN_IN_TEAM; i++) {
            playerList.add(Player.getPlayer(PlayerType.BATSMAN));
        }
        for(int i=0; i<MatchConstants.NUMBER_OF_ALLROUNDERS_IN_TEAM; i++) {
            playerList.add(Player.getPlayer(PlayerType.ALLROUNDER));
        }
        for(int i=0; i<MatchConstants.NUMBER_OF_BOWLERS_IN_TEAM; i++) {
    ...
    public int getNextBatsmanNumber() {
        if(nextBatsmanNumber == MatchConstants.NUMBER_OF_PLAYERS_IN_TEAM) return MatchConstants.ALL_OUT;
    ...
    public int getNextBowlerNumber(int currentBowlerNumber) {
        while(true) {
            int nextBowlerNumber = (int) (Math.random() * (MatchConstants.NUMBER_OF_ALLROUNDERS_IN_TEAM + MatchConstants.NUMBER_OF_BOWLERS_IN_TEAM))
                + MatchConstants.NUMBER_OF_BATSMEN_IN_TEAM;
            if (nextBowlerNumber == currentBowlerNumber) continue;
            else if (Math.round(players.get(nextBowlerNumber).getOversBowled()) == MatchConstants.NUMBER_OF_OVERS_PER_BOWLER) continue;
